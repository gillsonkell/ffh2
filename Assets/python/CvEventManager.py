...
# FfH Card Game: end
  ...
  def onGameStart(self, argsList):
    ...
    strSetData = { 'DragonWarrior': 5, 'Merc': -1, 'FinalWar': -1, 'val1': -1, 'val2': -1, 'val3': -1, 'orthus': CyGame().getSorenRandNum(20, "Orthus Turn") + 180 }
    ...
    if not CyGame().isUnitClassMaxedOut(gc.getInfoTypeForString('UNITCLASS_ORTHUS'), 0):
      if not CyGame().isOption(gc.getInfoTypeForString('GAMEOPTION_NO_ORTHUS')):
        ...
        if bOrthus:
          ...
          orth = cf.addUnit(iUnit)
          if orth != -1:
            turns = 13
            orth.setImmobileTimer(turns)
            bPlayer = gc.getPlayer(gc.getBARBARIAN_PLAYER())
            newUnit = bPlayer.initUnit(gc.getInfoTypeForString('UNIT_AXEMAN'), orth.getX(), orth.getY(), UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
            newUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_HERO'), True)
            newUnit.setName('Right Hand of Orthus')
            newUnit.finishMoves()
            newUnit.setImmobileTimer(turns)
            cf.equip(newUnit)
            iArmy = CyGame().getSorenRandNum(6, "Orthus Army")
            for i in range(iArmy):
              newUnit = bPlayer.initUnit(gc.getInfoTypeForString('UNIT_WARRIOR'), orth.getX(), orth.getY(), UnitAITypes.UNITAI_ATTACK, DirectionTypes.DIRECTION_SOUTH)
              newUnit.setName('Minion of Orthus')
              newUnit.setExperience(6,-1)
              newUnit.finishMoves()
              newUnit.setImmobileTimer(turns)
...
#Conquestmode for Religion Heros      
    ...
    if unit.getUnitCombatType() == gc.getInfoTypeForString('UNITCOMBAT_MOUNTED'):
      ...
      if iNum > 1:
        unit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_EMPOWER1'), True)
        if iNum > 2:
          unit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_EMPOWER2'), True)
          if iNum > 3:
            unit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_EMPOWER3'), True)
            if iNum > 4:
              unit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_EMPOWER4'), True)
              if iNum > 5:
  ...
  def onUnitBuilt(self, argsList):
    ...
    if unit.getUnitType() == gc.getInfoTypeForString('UNIT_KELLER'):
      sCityInfo[ 'OBELISK' ] = 2
      city.setScriptData(cPickle.dumps(sCityInfo))
      if city.getNumBuilding(gc.getInfoTypeForString('BUILDING_MONUMENT')) > 0:
        ## unit.setName("Magician of " + city.getName())
...
#UNITAI for Terraformers
  ...
  def onUnitKilled(self, argsList):
    ...
    if (unit.isAlive() and unit.isImmortal() == False):
      ...
      if unit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_SPIRIT_GUIDE')) and unit.getDuration() < 1:
...
#     pUnit.getArtInfo(pPlayer.getCurrentEra(),eraType).setScale(pUnit.getArtInfo(pPlayer.getCurrentEra())*1.35,eraType)
  ...
  def onUnitPillage(self, argsList):
    ...
    # CyInterface().addMessage(pUnit.getOwner(),True,25,"Pillaging!",'AS2D_UNIT_BUILD_UNIT',1,'Art/Interface/Buttons/Units/Crusader.dds',ColorTypes(8),pUnit.getX(),pUnit.getY(),True,True)
    
    # mtk: penalize human players from pillaging other players
    if pPlayer.isHuman() and pPlot.isOwned() and pPlot.getOwner() != pUnit.getOwner():
      iDam = 20 - pUnit.getLevel() - pUnit.baseCombatStr()
      if iDam > 0:
        pUnit.setDamage( pUnit.getDamage() + 20, pUnit.getOwner() )
      ...
      if (iTechType == gc.getInfoTypeForString('TECH_INFERNAL_PACT') and iPlayer != -1):
        ...
        if iCount == 1:
            ...
            if pPlot.isWater() == False:
              if pPlot.getNumUnits() == 0:
                if pPlot.isCity() == False:
                  if pPlot.isImpassable() == False and not pPlot.isOwned():
          ...
          if (iInfernalPlayer != -1 and pBestPlot != -1):
            ...
            mPlayer = gc.getPlayer(iInfernalPlayer)
            eInfernalTeam.setVassal(iFounderTeam,true,true)
            ...
            pFounderPlayer = gc.getPlayer(iPlayer)
            ...
            # newUnit13 = pFounderPlayer.initUnit(gc.getInfoTypeForString('UNIT_SPECTRE'), pBestPlot.getX(), pBestPlot.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_NORTH)
...
#             triggerData = owner.initTriggeredData(iEvent, true, -1, city.getX(), city.getY(), iPlayer, city.getID(), -1, -1, -1, -1)
  ...
  def onCityAcquired(self, argsList):
    ...
    if gc.getPlayer(iPreviousOwner).getCivilizationType() == gc.getInfoTypeForString('CIVILIZATION_INFERNAL') and not pPlayer.isHasTech(gc.getInfoTypeForString('TECH_PASS_THROUGH_THE_ETHER')):
    ...
    if pCity.getNumRealBuilding(gc.getInfoTypeForString('BUILDING_PLANAR_GATE')) > 0:
      ...
      if CyGame().getSorenRandNum(10000, "Planar Gate") <= gc.getDefineINT('PLANAR_GATE_CHANCE') * iMult:
        ...
        if len(listUnits) > 0:
          ...
          sMsg = 'A ' + newUnit.getName() + ' comes through the planar gate at ' + pCity.getName() + '...'
          CyInterface().addMessage(iPlayer,True,25,sMsg,'AS2D_DISCOVERBONUS',1,gc.getUnitInfo(newUnit.getUnitType()).getButton(),ColorTypes(8),pCity.getX(),pCity.getY(),True,True)
          ...
          if iUnit == gc.getInfoTypeForString('UNIT_MOBIUS_WITCH'):
            promotions = [ 'PROMOTION_AIR1','PROMOTION_BODY1','PROMOTION_CHAOS1','PROMOTION_DEATH1','PROMOTION_EARTH1','PROMOTION_ENCHANTMENT1','PROMOTION_ENTROPY1','PROMOTION_FIRE1','PROMOTION_LAW1','PROMOTION_LIFE1','PROMOTION_MIND1','PROMOTION_NATURE1','PROMOTION_SHADOW1','PROMOTION_SPIRIT1','PROMOTION_SUN1','PROMOTION_WATER1','PROMOTION_DIMENSIONAL1','PROMOTION_ICE1' ]
            newUnit.setLevel(4)
