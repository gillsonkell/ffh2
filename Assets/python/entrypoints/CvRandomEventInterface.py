...
def canInitial(argsList):
  ...
  if pPlayer.getGold() * 10 >= CyGame().getPlayerScore(iPlayer) * iCache:
    iGameTurn = CyGame().getGameTurn()
    iCycle = 25
    for i in range(10):
      if (i * iCycle) + 25 == iGameTurn:
        return True
...
def canalchemylab50(argsList):
  ...
  pCity = pUnit.plot().getPlotCity()
  strSetData = cPickle.loads(pCity.getScriptData())
  if ( ( CyGame().getGameTurn() - strSetData['BUILDING_ALCHEMY_LAB'] ) * pCity.getPopulation() ) / 15 > 49:
    return True
...
def alchemylab50(argsList):
  ...
  cf.pay(pCity,'BUILDING_ALCHEMY_LAB',50,iPlayer,'alchemy lab')
  
def doWyrmling(argsList):
  kTriggeredData = argsList[0]
  bPlayer = gc.getPlayer(kTriggeredData.ePlayer)
  pCity = bPlayer.getCity(kTriggeredData.iCityId)

  newUnit = bPlayer.initUnit(gc.getInfoTypeForString('UNIT_WYRMLING'), pCity.getX(), pCity.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)

def canTriggerGoblinKing(argsList):
  kTriggeredData = argsList[0]
  pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
  pUnit = pPlayer.getUnit(kTriggeredData.iUnitId)
  if pUnit.getLevel() < 12:
    return False
  if pUnit.getUnitType() == gc.getInfoTypeForString('UNIT_GOBLIN_ELITE'):
    return True
  return False

def doGoblinKing(argsList):
  kTriggeredData = argsList[1]
  pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
  pUnit = pPlayer.getUnit(kTriggeredData.iUnitId)
  pUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_STEALTH'), False)
  pUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_HIDDEN'), False)
  pUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_NOBILITY'), True)

def canTriggerAncientGrigori(argsList):
  kTriggeredData = argsList[0]
  pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
  pUnit = pPlayer.getUnit(kTriggeredData.iUnitId)
  if pUnit.getLevel() < 15:
    return False
  if pUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_ANCIENT')):
    return False
  return False

def canIE17(argsList):
  iEvent = argsList[0]
  kTriggeredData = argsList[1]
  iPlayer = kTriggeredData.ePlayer
  pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
